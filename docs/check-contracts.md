...
python scripts/check-contracts.py \
  --backend-dir backend \
  --settings education_experiment_platform.settings.test
```
...
| `--settings` | `education_experiment_platform.settings.test` | 指定 Django settings 模块。 |
...
1. 未找到同时定义 `ErrorCode` 与 `ERROR_CODES` 的模块。
2. `CELERY_BEAT_SCHEDULE` 为空。
3. 缺少 `docker-compose.yml`。
4. 缺少 `docker-compose.prod.yml`。

这些结果反映的是当前仓库状态，不是脚本错误。后续相关切片补齐后，对应 `FAIL` 应转为 `PASS`。
