...
DEFAULT_SETTINGS = "education_experiment_platform.settings.test"
...
def check_compose(report: Report, settings_queues: list[str]) -> None:
    ...
    for filename in ("docker-compose.yml", "docker-compose.prod.yml"):
        ...
        if env_queues is None:
            report.fail(f"{filename} 的 CELERY_TASK_QUEUES", "worker 环境变量未声明")
        elif set(worker_queues) != set(env_queues):
            report.fail(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Django settings 模块，默认 education_experiment_platform.settings.test",
    )
