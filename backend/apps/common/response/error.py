...
class ApiError(exceptions.APIException):
    """Base exception for stable API API errors."""
...
def _map_exception(exc: Exception, status_code: int) -> tuple[str, str, bool]:
    if isinstance(exc, ApiError):
        codes = exc.get_codes()
        return str(codes if isinstance(codes, str) else exc.default_code), str(exc.detail), exc.retryable
...
def bad_request_view(request: HttpRequest, exception=None):
    return _django_error(request, code="VALIDATION_ERROR", message="请求无效", status_code=400)


def permission_denied_view(request: HttpRequest, exception=None):
    return _django_error(request, code="PORTAL_ACCESS_DENIED", message="无权执行此操作", status_code=403)


def not_found_view(request: HttpRequest, exception=None):
    return _django_error(request, code="RESOURCE_NOT_FOUND", message="资源不存在", status_code=404)
