...
def drf_exception_handler(exc: Exception, context: dict[str, Any]) -> JsonResponse:
    ...
    if isinstance(request, HttpRequest):
        ...
        trace_id = request.headers.get("X-Trace-ID", trace_id)
    ...
    if isinstance(exc, Http404):
        return error_response(RESOURCE_NOT_FOUND, request_id, trace_id=trace_id)
    if isinstance(exc, PermissionDenied):
        return error_response(FORBIDDEN, request_id, trace_id=trace_id)

    response = exception_handler(exc, context)
    if response is None:
        return error_response(INTERNAL_ERROR, request_id, trace_id=trace_id)
